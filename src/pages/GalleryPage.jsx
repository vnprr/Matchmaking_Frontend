...
import {
    fetchUserImages, uploadImage, cropImage, updateOrder, deleteImage, setMainImage
...
const GalleryPage = () => {
    ...
    const loadImages = async () => {
        ...
        try {
            ...
            setImages(imgs);
            // Ustaw pierwszy obrazek jako domyślnie wybrany
            if (imgs.length > 0) {
    ...
    // Dodatkowy efekt do aktualizacji wybranego obrazka po zmianach w galerii
    useEffect(() => {
        if (images.length > 0 && !selectedImage) {
            setSelectedImage(images[0]);
        } else if (selectedImage) {
            // Sprawdź czy wybrany obrazek nadal istnieje po usunięciu
...
// Drag & drop zmiana kolejności
    ...
    const handleSetMainImage = async (imageId) => {
        ...
        try {
            ...
            await loadImages(); // Odświeżenie listy zdjęć
            setError('');
        } catch (e) {
            console.error('Błąd ustawiania zdjęcia jako główne:', e);
            const errorMessage = e.response?.data || e.message || 'Nieznany błąd';
            setError(`Błąd podczas ustawiania zdjęcia jako główne: ${errorMessage}`);
        }
        setLoading(false);
    };
                    ...
                    {images.map((img, idx) => (
                        ...
                        >
                            <Image
                                ...
                                objectFit="cover"
                                cursor="pointer"
                                border={selectedImage && selectedImage.id === img.id ? "2px solid #3182ce" : "none"}
                                borderRadius="sm"
                            ...
                            <Button
                                size="sm"
                                colorScheme={img.isMain ? "blue" : "gray"}
                                onClick={() => handleSetMainImage(img.id)}
                                isDisabled={img.isMain}
                                mr={1}
                            >
                                {img.isMain ? "Główne" : "Ustaw jako główne"}
                            </Button>
